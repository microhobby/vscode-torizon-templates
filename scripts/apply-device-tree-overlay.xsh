...
##
# This script is used to list the applied device tree overlays
# WARNING:
...
_dtos = sys.argv[4] if len(sys.argv) >= 4 else None

if _login is None or _pass is None or _ip is None or _dtos is None:
    print("invalid arguments")
    os._exit(69)


def __apply_dev_list():
    return !(node ./node/applyDeviceTreeOverlays.mjs \
                @(_ip) \
                @(_login) \
                @(_pass) \
                @(_dtos))
...
dev_list = run_command_with_wait_animation(__apply_dev_list)
...
if dev_list.returncode != 0:
    ...
    print("")
    print("🌳 :: DEVICE TREE OVERLAYS APPLIED :: 🌳")
    print("")
